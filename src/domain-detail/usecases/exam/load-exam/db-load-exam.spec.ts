...
import { LoadExamRepositorySpy } from '@/domain-detail/test/mock-db-exam'
import { throwError } from '@/domain-policy/test'
...
describe('DbLoadExam Usecases', () => {
  test('Should call LoadExamRepository', async () => {
    ...
    await sut.load()
    expect(jest.spyOn(loadExamRepositorySpy, 'load')).toHaveBeenCalled()
  })
